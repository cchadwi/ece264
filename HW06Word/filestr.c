...
int countWord(char * filename, char * word, char * line, int size)
{
  ...
  int sum = 0;
  char *loc; //location of line (needs to be pointer to do strstr)
  while(fgets(line, size, fptr) != NULL)
  {
    loc = line;
    while(loc != NULL)
    {
      loc = strstr(loc, word); //finds first occurance
      if(loc != NULL)
      {
        sum++; 
        loc = loc + strlen(word); //calc length
      }
    }
  }
  fclose(fptr);
  return sum; //needed to change from 0 since that would only return 0, not the expected sum output
}
#endif
